...
void update(int id)
{
	...
	if (time > nextTime)
	{
		unique_ptr<Object> meteorite(make_unique<Object>());

		// 球を表すメッシュをセット
		meteorite->getMesh() = Mesh::createSphere(1.0, 12);
		meteorite->getMesh().update();
		...
		// マテリアルはとりあえず全部白
		Material material;
		material.setDiffuseColor({ 1.0f, 0.5f, 0.75f, 1.0f });
...
void setup()
{
	...
	// ライト環境光(白)
	// ライト鏡面反射光(白)
	light.setPosition({ 0.0f, -1.0f, 0.0f, 0.0f });
